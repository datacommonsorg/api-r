...
1. Follow the instructions [in the next section](https://datacommons.readthedocs.io/en/latest/started.html#enable-the-data-commons-api) to enable the Data Commons API for your GCP project.
...
- **`outgoing`** - optional boolean flag indicating whether the property is directed away from the given nodes. TRUE by default
- **`valueType`** - optional string identifying the node type to filter the returned property values by. NULL by default.
- **`limit`** - optional integer indicating the maximum number of property values returned aggregated over all given nodes. 100 by default.
...
When `dcid`s is specified as a column, the return value is a column with the same format as that returned by `GetPropertyValues`. Let's call `GetPlacesIn` to get all counties within California.
...
Notice that both `state_name` and `county` are columns with lists. `unnest` will automaticall flatten all columns that have lists in their cells.
...
- **`populationType`** - string identifying the population type of the `StatisticalPopulation`.
- **`constraintsPVMap`** - optional named list mapping constraining property to the value that the `StatisticalPopulation` should be constrained by. Empty by default.

When a column is provided to `dcids`, the return value is a column with each cell populated by a single `dcid` and not a list. This is because the combination of `dcids`, `populationType`, and optionally `constraintsPVMap` always map to a unique `StatisticalPopulation` `dcid` if it exists.
...
- **`measuredProperty`** - string identifying the measured property.
- **`statsType`** - string identifying the statistical type for the observation.
- **`observationDate`** - string of the associated observation date in ISO8601 format.
- **`observationPeriod`** - string specifying the observation period. Required if the Observations you are querying for specify this field. NULL by default.
- **`measurementMethod`** - string specifying the measurement method. Required if the Observations you are querying for specify this field. NULL by default.
