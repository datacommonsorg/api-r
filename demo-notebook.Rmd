...
This tutorial introduces the Data Commons open knowledge graph and discusses how to programmtically access its data through the R API Client.
...
devtools::install_github("datacommonsorg/api-r", quiet=TRUE)
library(datacommons, quiet=TRUE)
```

Using the Data Commons API requires you to provision an API key on GCP. Follow these steps to create your own key. To get an API key, follow the instructions [here](https://datacommons.readthedocs.io/en/latest/started.html#creating-an-api-key) to get your API key.

Replace `Sys.getenv("API_KEY")` with your API key, such as "123456". Then, uncomment and run the following block.
